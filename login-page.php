<?php
    include("login-handler.php");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
    <link rel = "stylesheet" type = "text/css" href = "styles.css">
    <script type = "text/javascript"  src = "scripts.js" async></script>
    <title>Secret Diary</title>
</head>
<body>
    <div class = "container">
        <!-- Title/Infor Section -->
        <h1>Secret Diary</h1>
        <p class = "lead"><strong>Store your thoughts permanently and securely</strong></p>
        <p id = "description"> Interested? Sign up now. </p>
        <div id = "error-section"><?php echo $errorMessage?></div>

        <!-- Login/Signup Section -->
        <form method = "post">
            <div class="form-group">
                <input type="email" class="form-control" id = "email" name = "email" placeholder = "Email Address">
                <small >We'll never share your email with anyone else.</small>
            </div>
            <div class="form-group">
                <input type="password" class="form-control" id = "password" name = "pass" placeholder = "Password">
            </div>
            <div class="form-group form-check">
                <input type="checkbox" class="form-check-input" name="logged-in">
                <label class="form-check-label" for="stay">Stay logged in.</label>
            </div>
            <input id = "login-type" name = "login-type" type="hidden" value = "Sign Up">
            <button id = "submit" type="submit" class="btn btn-success">Sign Up</button>
        </form>
        <br>
        <a id ="login-toggle">Log In</a>
    </div>
</body>
</html>